  ...
  return (
    <div>
      <Table loading={loading} dataSource={dataSource || []} pagination={false} columns={isAction ? myColums : columns}
             rowKey={record => record?.activity_id || record?.id || record?.extra_add_id||record?.extra_deduction_id||record?.stu_number||record?.activity_semester_id||record?.score_id||record?.gpaid} scroll={{ x: 1200 }}/>
