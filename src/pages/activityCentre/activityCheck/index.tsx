...
const ActivityCheck = () => {
  ...
  const columns = [
    ...
    },
    {
      ...
      key: 'status',
      //0 申请中     1 申请通过    2 申请失败
      render: (accord: API.ActivityRes) => <>
        <Select defaultValue={checkState(accord)} style={{ width: 120 }}
    ...
    },
    {
      title: '报名时间',
      key: 'activity_id',
      render: (accord: API.ActivityRes) => <>{dateChange(accord?.sign_up_start_time)}
      <br/>
      ~{dateChange(accord?.sign_up_end_time)}</>,
    },
    {
      title: '活动时间',
      key: 'activity_id',
      render: (accord: API.ActivityRes) => <>{dateChange(accord?.activity_start_time)}
        <br/>
        ~{dateChange(accord?.activity_end_time)}</>,
    },
    {
      title: '操作',
      key: 'action',
